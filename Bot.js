...
function bots(VK, token) {  
	...
	vk.setOptions({
		token: '44e4517f418ec897577bd149f4a6809384135c8f07bf2078a000ca3f979e88e05528f64839da2c8a90ae4',
		apiMode: 'parallel',
		pollingGroupId: 121266716
	});  
	/*==========================================================================================================*/
	    ...
	    if(!uid[1][message.user]){ 
			...
			if(acc[aid] != 1){
				return message.send(`
					...
					🔹 Чтобы играть в боте, 
					🔹 Вам нужно узнать команды - "Команды"
					❗ Наша беседа https://vk.me/join/AJQ1dxC3oQ8UAwQQbaT8OpC6
				`)
			}
			////////////////////  
...
const heros = {
	"1": {
		group: "Маг",
		text: `1. Маг — герой, посвятивший свою жизнь изучению древних мистических таинств и искусств. Он не способен сражаться в первых рядах, но при должной поддержке успеет нанести огромный урон, пока противники будут тщетно пытаться добраться до него.`
	},
	"2": {
		group: "Техник",
		text: `2. Техник — герой, чьи технологии и машины позволяют выполнить любую задачу, какую только можно придумать. Сам по себе он довольно хил, но мощная броня и совершенное оружие решают эту проблему за него. `
	},
	"3": {
		group: "Воин",
		text: `3. Воин — герой, для которого долг и честь — краеугольные камни жизни. Закалённый в боях, он может принять на себя главный удар вражеских сил, при этом сокрушая противников по всем фронтам.`
	},
	"4": {
		group: "Варвар",
		text: `4. Варвар — герой, которого не волнует ничего, кроме его жажды сражений, крови и побед. Он будет биться с врагами лицом к лицу до тех пор, пока не погибнет он, или, что куда более вероятно, они. `
	},
	"5": {
		group: "Командир",
		text: `5.Командир — герой, присутствие которого на поле боя само по себе воодушевляет союзников. До тех пор, пока его не победили, его отряды будут биться с удвоенным пылом и отвагой — а победить его весьма трудно. `
	},
	"6": {
		group: "Божество",
		text: `6. Божество — герой, само существование которого не может быть объяснено, а пределы его мощи не могут быть ограничены. Сражаясь с ним, враг может быть обманут кажущейся хрупкостью — перед тем, как единственный удар покончит с ним.`
	}
}
...
function check(user){
	if(user.hero.id == false) return `\n‼ Упс...‼\n Вы не зарегистрированы ! Перед началом игры, вам нужно создать персонажа.\n\n1&#8419; Этап. Выбор персонажа.\n&#9193; "Герои" - список персонажей.`; 
	if(user.hero.prefix == false) return `‼ Упс...\n Вы не зарегистрированы ! Перед началом игры, вам нужно придумать имя для героя.\n\n2&#8419; Этап. Придумайте имя.\n&#9193; "Назвать [имя]".Мат в имени - БАН!`;
} 

function check_id(id){
	if(id != 1) return `‼ Упс...\n‼ Эта команда доступна в только в https://vk.com/gamebotofficial
	`;
} 
